...
function PostListCard(props) {
	...
	const togglePostImageView = () => {
		props.imageClick(props.post.fileSrc);
	};
	...
	const openPostButtonHandler = async (e) => {
		...
		else if (e.target.id === "postImage") togglePostImageView();
	...
	return (
		<div
			...
			className="bg-base outline-base hover:outline-white  p-3 my-4 overflow-hidden rounded-xl  outline outline-1  hover:outline hover:outline-1 "
		>
			<div className="grid grid-cols-12 gap-4 overflow-hidden">
				<div className="col-span-9">
					...
					<div className="text-tprimary truncate">
						{props.post.content}
					</div>
				</div>
